...
In the router's `SetSysTimeCfg` function, `ntpServer` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `ntpServer` to execute arbitrary code.
