...
In the router's `langSwitchBack` function, `langSelectionOnly` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `langSelectionOnly` to execute arbitrary code.
