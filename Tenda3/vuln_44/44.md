...
In the router's `AdvSetMacMtuWan` function, `serverName` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `serverName` to execute arbitrary code.
