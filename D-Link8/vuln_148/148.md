...
In `cgi_create_import_users` function, `overwrite、app` is directly passed by the attacker, so we can control the `overwrite、app` to attack the OS.
...
We set `app` as **'`utelnetd -p 8937 -l /bin/sh`'** , and the router will excute it,such as:
